...
static bool AttachToNative(HWND window_handle, HDC& out_device_context, HGLRC& out_original_context, HGLRC& out_render_context);
...
bool Backend::Initialize(const char* window_name, int width, int height, bool allow_resize)
{
	...
	geode::log::debug("dick shit fart explosion {}", (uint64_t)data.get());
    ...
    HWND window_handle = FindWindowA(nullptr, "Geometry Dash");

    geode::log::debug("[Backend::Initialize] HWND {}", (int64_t)window_handle);
	...
	// Attach the OpenGL context.
	if (!AttachToNative(window_handle, data->device_context, data->original_context, data->render_context))
	{
        geode::log::debug("[Backend::Initialize] SHUTTING WINDOW");
	...
	geode::log::debug("dick shit fart explosione {}", (uint64_t)data.get());
	return true;
}
...
{
	RMLUI_ASSERT(data);
    geode::log::debug("[Backend::GetRenderInterface] {}", (int64_t)&data->render_interface);
...
void Backend::BeginFrame()
{
    ...
    // if (!wglMakeCurrent(data.get()->device_context, data.get()->render_context))
	// {
	// 	geode::log::error("[AttachToNative]: ERROR: Unable to make rendering context current.");
    //     return;
	// }
...
void Backend::PresentFrame()
{
    ...
    // if (!wglMakeCurrent(data.get()->device_context, data.get()->original_context))
	// {
	// 	geode::log::error("[AttachToNative]: ERROR: Unable to make rendering context current.");
	// }
...
static bool AttachToNative(HWND window_handle, HDC& out_device_context, HGLRC& out_original_context, HGLRC& out_render_context)
{
    ...
    HGLRC original_context = wglGetCurrentContext();

    geode::log::debug("[AttachToNative] HDC {}", (int64_t)device_context);

	if (!device_context)
	{
		geode::log::error("[AttachToNative]: ERROR: Could not get device context.");
		return false;
	}
	...
	// if (!pixel_format)
	// {
	// 	geode::log::error("[AttachToNative]: ERROR: Could not choose 32-bit pixel format.");
	// 	return false;
	// }

	// if (!SetPixelFormat(device_context, pixel_format, &pixel_format_descriptor))
	// {
	// 	geode::log::error("[AttachToNative]: ERROR: Could not set pixel format.");
	// 	return false;
	// }
	...
	if (!render_context)
	{
		geode::log::error("[AttachToNative]: ERROR: Could not create OpenGL rendering context.");
		return false;
	}
    ...
    out_original_context = original_context;
...
static void DetachFromNative(HWND window_handle, HDC device_context, HGLRC render_context)
{
    // TODO: make sure original context is back here?
	// Shutdown OpenGL
	// if (render_context)
	// {
